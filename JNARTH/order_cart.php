...
<li><a href="#footers" id="down"><i class="fa fa-angle-down"></i></a></li>
    ...
    <?php
     ...
     if(mysqli_num_rows($result) > 0){
        ...
        $row = mysqli_fetch_assoc($result);
        echo '<h1 class="section-p1">Your OrderID: ' . $row["order_id"] . '</h1>';
     }else{
         echo '<h1 class="section-p1"> Your OrderID : 0 </h1>';
...
$grand_total = 0;
                ...
                $query = "SELECT * FROM `orders` AS ors
                JOIN `add_to_cart` AS ass ON ors.user_id = ass.u_id AND ors.product_id = ass.product_id
    ...
    <h3></h3>
    <?php
    echo "<p>Total Price: $grand_total USD</p>";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $statusss = $row['status'];
        echo "<p>Order Status: $statusss</p>";
        if ($status == "Pending") {
            echo "<a href='order_cart.php' class='full-width-button'>Place Order</a>";
        } else {
            echo "<p>Payment Method: $status</p>";
        }
    } else {
