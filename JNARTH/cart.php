...
$grand_total = 0;
                ...
                $query = "SELECT * FROM `add_to_cart` WHERE u_id = '$userID'  AND c_status = 0";
                $result = mysqli_query($conn, $query);
$subtotal = 0;
if(mysqli_num_rows($result) > 0) {
                while($row = mysqli_fetch_assoc($result)){
                  ...
                  <td><a href="#" onclick="confirmDelete(<?php echo $row['c_id']; ?>)"><i class="fa fa-times-circle"></i></a></td>
        <td><img src="image/<?php echo $row["c_image"]; ?>" alt="Error"></td>
        <td><?php echo $row["c_name"]; ?></td>
        <td><?php echo $row["c_size"]; ?></td>
        <td>$<?php echo $row["c_price"]; ?></td>
        <td>
                    <input type="hidden" value="<?php echo $row['c_id']; ?>" name="update_id">
                    <input type="number" name="update_quantity" id="Quantity" min="1" value="<?php echo $row["c_quantity"]; ?>">
                    <button type="submit" class="update-button" name="update_product_quantity">Update</button>
                    </form>
    </td>
    <td>$<?php echo $subtotal = $row["c_price"] * $row["c_quantity"]; ?></td>
                ...
                <?php
                  $grand_total += $subtotal;
                    ...
                    echo "<tr><td colspan='7' align='center'>No items in your cart.</td></tr>";
                }
                ?>
                <?php
...
if (isset($_POST['update_product_quantity'])) {
    ...
    $result = mysqli_query($conn, "UPDATE `add_to_cart` SET c_quantity = '$update_value'   WHERE c_id = '$update_id'");
  ...
  <script>
    function confirmDelete(c_id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = `delete_cart.php?c_id=${c_id}`;
            }
        });
    }
    </script>
    ...
    <section id="coupon" class="section-p1">
        ...
        <div class="carttotal">
            ...
            <button onclick="window.location.href='place_order.php'">Proceed to Checkout</button>
